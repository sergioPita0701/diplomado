...
$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.'Lista de Calificaciones por Modulo - '.$nombre, PDF_HEADER_STRING);
...
$tabla='<table border="1" cellspacing="3" cellpadding="4">
    ...
    $no=1;
    foreach($listacalificaciones as $filita ) {
$tabla.=        
    '<tr>
        <td>'.$no++.'</td>
        <td>'.$filita['ciD'].'</td>
        <td align="center">'.$filita['nombreD'].' '.$filita['apellidoPaternoD'].'</td>
        <td align="center">'.$filita['nombreM'].'</td>
        <td align="center">'.$filita['nota'].'</td>
        <td align="center">'.$filita['establece_nota'].'</td>
    </tr>';
    }
