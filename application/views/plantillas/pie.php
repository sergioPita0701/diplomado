...
<script type="text/javascript">
  ...
  function buscarci() {
    ...
    if (ci.length > 0) {
      $.ajax({
        type: 'post',
        url: "http://localhost:8080/diplomado/index.php/diplomante/buscarDiplomante",
        data: {
          txtci: ci,
        },
  ...
  function crearDiplomante() {
    ...
    if (nombre.length == 0 && apepaterno.length == 0 && profesional == null) {
      $("#vacionombre").html('<p class="alert alert-danger"><small>Espere!!, El campo nombre es requerido.</small></p>');
      $("#vacioapepaterno").html('<p class="alert alert-danger"><small>Espere!!, El campo Apellido Paterno es requerido.</small></p>');
      $("#vacioprofesion").html('<p class="alert alert-danger"><small>Espere!!, El campo Profesion es requerido.</small></p>');
    } else {
      if (nombre.length != 0 && apepaterno.length == 0 && profesional == null) {
        $("#vacioapepaterno").html('<p class="alert alert-danger"><small>Espere!!, El campo Apellido Paterno es requerido.</small></p>');
        $("#vacioprofesion").html('<p class="alert alert-danger"><small>Espere!!, El campo Profesion es requerido.</small></p>');
            ...
            if (nombre.length == 0 && apepaterno.length != 0 && profesional == null) {
              ...
              if (nombre.length == 0 && apepaterno.length == 0 && profesional != null) {
                ...
                if (nombre.length == 0 && apepaterno.length != 0 && profesional != null) {
                  ...
                  $.ajax({
                    type: 'post',
                    url: "http://localhost:8080/diplomado/index.php/diplomante/crearDiplomante",
                    data: {
                      txtci: ci,
                      txtNombreD: nombre,
                    ...
                    success: function(data) {
                      ...
                      if (data == "false") {
                        $("#mensaje").html('<p class="alert alert-danger">No es posible registar al Participante, revise sus datos.</p>');
                      } else {
  ...
  function editardiplomante() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/diplomante/editarDiplomante",
      data: {
        textoCiDi: cid,
      ...
      success: function(data) {
        ...
        if (data == "false") {
          ...
          $(".msjnotificacion").html('<p class="alert alert-success">No es posible Editar datos del Participante, revise sus datos.<p>').fadeIn();
        } else {
          $("#modalEditDP .close").click();
          $("#dpci").text(data[0]['ciD']);
          $("#dpnombrecompleto").text(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
          $("#dpciudad").text(data[0]['ciudadD']);
          $("#dpprofesion").text(data[0]['nombreP'] + "(" + data[0]['gradoAcademicoP'] + ")");
          $(".msjnotificacion").html('<p class="alert alert-success">Se Edito datos del Participante.<p>').fadeIn();
      ...
      // window.location.load('http://localhost:8080/diplomado/index.php/inscripcion/detalleDiplomante_inscrito/'+data);
      // window.location;
...
<script>
  ...
  function editardinscripcion() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/inscripcion/editarInscripcion",
      data: {
        textociDI: cid,
        textoNumI: numi,
      ...
      success: function(data) {
        ...
        if (data == "false") {
          ...
          $(".msjnotificacion").html('<p class="alert alert-danger">No es posible Editar datos de Inscripcion del Participante, revise sus datos.<p>').fadeIn();
        } else {
          $("#modalDatosInscripcion .close").click();
          ...
          $(".msjnotificacion").html('<p class="alert alert-success">Se Edito datos de Inscripcion del Participante.<p>').fadeIn();
        }
      },
      complete: function() {
        setTimeout(function() {
...
<script type="text/javascript">
  ...
  //     {
  //       type:'post',
  //       url:"http://localhost:8080/diplomado/index.php/inscripcion/fechaActual",
  //       data:{
...
<script>
  ...
  function terminarfInscripciones() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/inscripcion/habilitar_inscripcion",
      data: {
        fechainscripcion: fechainsc
      },
      success: function(data) {
        if (data == true) {
          $("#mnsjFechaInsc").html('<p class="alert alert-success"><span class="glyphicon glyphicon-edit" ></span><small> Se guardó fecha satisfactoriamente </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();

        } else {
          $("#mnsjFechaInsc").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> No se pudo realizar Cambios!! </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();
        }
      },
      complete: function() {
        setTimeout(function() {
  ...
  $(document).ready(function() {
    $("select[name=modulosele]").change(function(params) {
      ...
      if (nummodulo == '') {
        ...
        $.ajax({
          type: 'post',
          url: "http://localhost:8080/diplomado/index.php/paralelo/get_paralelosde_modulo",
          data: {
            modulo: nummodulo
          },
          success: function(data) {
            ...
            $.each(data, function(key, registro) {
              ...
              $('#paralelosele').append('<option value=' + registro.idParalelo + '>' + registro.nombre_paralelo + '</option>');
            });
          }
        });
      }
    ...
    if (insc == '' && diplo == '') {
      ...
      $("#formasigmodulo").validate({
        ...
        submitHandler: function(form) {
          ...
          $.ajax({
            type: "POST",
            url: "http://localhost:8080/diplomado/index.php/modulodiplomante/asignarModuloaDiplo",
            // data: dataString,
            data: {
  ...
  $(document).ready(function() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/diplomante/inscribirModulo",
      data: {
        txtci: ci,
      },
      success: function(data) {
        ...
        if (data == 0 || data == 'nohayasignamd') {
          ...
          if (data == "aprobomodulos") {
            ...
            $.ajax({
              type: 'post',
              url: "http://localhost:8080/diplomado/index.php/diplomante/inscribirMonografia",
              data: {
                txtci: ci,
              },
              success: function(data) {
  ...
  function AsignarModulo() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        console.log(data);
        if (data == 0 || data == 'nohayasignamd') {
          $.getJSON('http://localhost:8080/diplomado/index.php/diplomante/get_primer_modulo', function(data) {
            // data=JSON.parse(data);
            numeroMod = data[0]['numeroM'];
            nombreMod = data[0]['nombreM'];
            console.log(nombreMod);
            console.log(numeroMod);
            $("#modulotoca").html('<p class="alert alert-success"><small>El diplomante es Nuevo, le toca cursar el Primer Modulo:' + data[0]['nombreM'] + '</small></p>');
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (data == "notienemono") // data[0]['idMonografia']==null,data.empty(),
        {
          ...
          $.getJSON('http://localhost:8080/diplomado/index.php/academicoseleccionado/getTodosAcademicoSelec', function(data) {
            $.each(data, function(key, registro) {
          ...
          $.ajax({
            type: 'post',
            url: "http://localhost:8080/diplomado/index.php/defensa/getDefensa_porci",
            data: {
              cidiplo: ci
            },
            success: function(data) {
              if (data == "vacio") {
                $("#mensajedefensa").html('<em>Registre su Defenza en el Menu de Monografias</em>');
              } else {
                data = JSON.parse(data);
          ...
          $("#tituloMono").html('<center><p class="alert alert-info"><small><strong>Titulo : </strong>' + data[0]['tituloMonografia'] + '</small></p></center>');
  ...
  $("#cambiarparalelo").click(function() {
    if (confirm('Los participantes seleccionados se cambiaran de Paralelo?')) {
      $("#participantes tbody tr").each(function(index) {
        var campoci, camporegistro, campochecked, campomodulonum, campoparalelo, fechaeditparalelo, respuesta;
        ...
        campoparalelo = $("#paraleloMenosUno").val(); //se puede hacer un if de esto para que no haga nad por q no hay mas pparlelos
        ...
        if (campoparalelo != null) {
          $.ajax({
            type: 'post',
            url: "http://localhost:8080/diplomado/index.php/modulodiplomante/editarParaleloDiplomante",
            data: {
          ...
          if (respuesta == 'noeditado') {
            $("#msjeditasigancion").html('<p class="alert alert-danger"><small>No se pudo cambiar de Paralelo a los Participantes seleccionados!!!</small></p>').fadeIn();
            // ("#mostrarocultar").show();
          } else {

            $("#msjeditasigancion").html('<p class="alert alert-success"><small>Los Participantes seleccionados se Reprogramaron al Paralelo seleccionado !!!</small></p>').fadeIn();
          }
        } else {
          $("#msjeditasigancion").html('<p class="alert alert-danger"><small>No existe otro Paralelo para realizar la operación!!!</small></p>').fadeIn();
          setTimeout(function() {
            $("#msjeditasigancion").fadeOut();
            top.location.reload(true);
          }, 2000);
...
<script>
  ...
  $("#obtenernota").click(function() {
    $("#tabla tbody tr").each(function(index) {
      ...
      if (camponota != '') {
        ...
        $.ajax({
          type: 'post',
          url: "http://localhost:8080/diplomado/index.php/calificacion/registrar_nota",
          data: {
            ciDiplo: campoci,
            nota: camponota,
        ...
        if (respuesta == 'nonota') {
          $("#msjnota").html('<p class="alert alert-danger"><small>No se registro calificacion!!!</small></p>').fadeIn();
          ("#mostrarocultar").show();
        } else {
        ...
        $("#msjnota").html('<p class="alert alert-danger"><small>Espere!! no se permiten valores nulos en los campo, el minimo valor aceptado es cero(0)</small></p>').fadeIn();
        $("#mostrarocultar").show();
      }
  ...
  $("body").on("click", "#tabla b", function(event) {
    ...
    $.ajax({
      type: "POST",
      url: "http://localhost:8080/diplomado/index.php/calificacion/kardex",
      data: {
        ciDiplomante: cisele
      },
      success: function(data) {
        data = JSON.parse(data);
        $('#datosciDiploKardex').html("<p class='text text-success'> CI : <strong>" + data[0]['ciD'] + "</strong></p>");
        $('#datosnombreDiploKardex').html("<p class='text text-success'>Nombre :<strong> " + data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD'] + " </strong></p>");
        // console.log(data);
        if (data != null && $.isArray(data)) {
          $.each(data, function(index, value) {
            $('#listabodykardex').append("<tr><td>" + value['nombreM'] + "</td><td>" + value['nota'] + "</td><td>" + value['establece_nota'] + "</td></tr>");
          })
        } else {
    ...
    $.ajax({
      type: "POST",
      url: "http://localhost:8080/diplomado/index.php/calificacion/kardexMonografia",
      data: {
        ciDiplomante: cisele
      },
      success: function(data) {
...
<script>
  $("body").on("click", "#listaAcademicos b", function(event) {
    ...
    $("#titulolabel").append('<small>Seleccionar a <strong>' + nombresele + '</strong> para asignarle un Rol</small>');
...
<script>
  ...
  $(document).ready(function() {
    ...
    if (ciAcadDoc == '') {
      $("#msjErr").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> Debe seleccionar un Academico de la lista de superior o del apartado <em>"Todos los Academicos"</em> </small><span class="glyphicon glyphicon-hand-up" ></span></p>');
    } else {
      $("#msjdocencia").hide();
      ...
      $("#formasigdoc").validate({
        ...
        submitHandler: function(form) {
          ...
          $.ajax({
            type: "POST",
            url: "http://localhost:8080/diplomado/index.php/docencia/crearDocencia_paralelo",
            data: {
              ciAcadDoc: ciAcadDoc,
              modulosele: modulosele,
              paralelosele: paralelosele,
            ...
            success: function(data) {
              ...
              if (data == 'noinsertado') {
                $("#msjdocencia").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-floppy-remove" ></span><small> No se puede Registrar Docencia!! El Paralelo ya cuenta con un Docente. </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();

              } else {
                $("#msjdocencia").html('<p class="alert alert-info"><span class="glyphicon glyphicon-ok" ></span><small> Docencia Registrada al Paralelo Exitosamente!!! </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
                $("#msjdocencia").show();
  ...
  $("body").on("click", "#listaDocencia b", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/docencia/obtenerDatos_docencia",
      data: {
        docencia: docencia
      },
      success: function(data) {
  ...
  function cambiarDetDocencia() {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/docencia/editarDatos_docencia",
      data: {
        docencia: docencia,
        fechaIni: fechaIni,
      ...
      success: function(data) {
        ...
        if (data == 'noeditadoDetDoc') {
          $("#msjEditDetDoc").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-floppy-remove" ></span><small> No se puede Editar Datos de Docencia!! </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();

        } else {
          $("#msjEditDetDoc").html('<p class="alert alert-info"><span class="glyphicon glyphicon-ok" ></span><small> Datos de Docencia Editada Exitosamente!!! </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
          // $("#msjEditTuto").show();
        }
      },
      complete: function() {
...
<script>
  ...
  $(document).ready(function() {
    ...
    $("#formasignartutoria").validate({
      ...
      submitHandler: function(form) {
        ...
        $.ajax({
          type: "POST",
          url: "http://localhost:8080/diplomado/index.php/tutoria/crearTutoriaMonografia",
          data: {
          ...
          success: function(data) {
            ...
            if (data == 'notuto') { //==
              ...
              // $("#msjtutoria").show();                    
              window.location.href = "http://localhost:8080/diplomado/index.php/tutoria/imprimirpdftutoria/" + $("#realizamono").val();
  ...
  $("body").on("click", "#listaTutorias b", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/tutoria/obtenerDatos_deTutoria",
      data: {
        realizamono: realizaMono
      },
      success: function(data) {
        data = JSON.parse(data);
        // console.log(data);
        $('#diplomanteTuto').text(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
  ...
  function cambiarTtutoria() {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/tutoria/editarTutoriaMonografia",
      data: {
      ...
      success: function(data) {
        ...
        if (data == "noeditTutoriaMono") {
          $("#msjEditTuto").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-floppy-remove" ></span><small> No se puede Editar Datos de Tutoria!! </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();

        } else {
          $("#msjEditTuto").html('<p class="alert alert-info"><span class="glyphicon glyphicon-ok" ></span><small> Datos de Tutoría Editada Exitosamente!!! </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
          $("#msjEditTuto").show();
        }
      },
      complete: function() {
...
<script>
  ...
  //     {
  //       type:'POST',
  //       url:"http://localhost:8080/diplomado/index.php/academicoseleccionado/getAcademicoSelec",
  //       data:{
  //         ciacad:aci,
  //       },
  //       success:function(data){
  ...
  function getseleccionarDiplo() {
    $('#nombreDef').html(''); //AQUI HACER AL SACAR EL DIPLOMANTE QUE VEA SI YA TIENE DEFENSA 1 SI ES ASI Y SU NOTA ES REPROBADO ASIGNARLE LA OPCION DE DEFENSA 2 EN EL IMPUT SI NO QUE APARESCA LA LA DEFESA 1 SEGUN SU NOTA HACER
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/monografia/getmonoDiplomante",
      data: {
        realizamono: mono,
      },
      success: function(data) {
        ...
        $('#nombreParticipante').val(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
  ...
  function getselecDiplopDefensa() { /// SIN USO NO FUNCIONA????
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/defensa/obtenerDefensa_porMono",
      data: {
        realizamono: mono
      },
      success: function(data) {
        ...
        if (data != 'vacio') {
          ...
          $('#nombreParticipante').val(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
          $('#titulomono').text(data[0]['tituloMonografia']);
          if (data[0]['notaDef'] == 0) { //hacer que tb cambien en la tabla monoo trabajar con la tabla aprobar =Def que no se si saca ese dato
            // $('#nombreDef').val(1);
            $('#nombreDef').append('<option value="1">Primera Defensa</option>');
            $('#tipdef').show();
            $('#fechdef').show();
            ...
            if (data[0]['notaDef'] >= 1 && data[0]['notaDef'] <= 60) { //REVISAR BIEN NO FUNCIONAAA!!!!! TIENE QUE CAMBIAR PORQUE AL APROBAR DEFENSA SE REGISTRA EN aprobarDef de la tabla defensa no en la tabla mono, hacer que tb cambien en la tabla monoo trabajar con la tabla aprobar =Def que no se si saca ese dato
              // $('#nombreDef').val(2);
              $('#nombreDef').append('<option value="2">Segunda Defensa</option>');
              $('#tipdef').show();
              $('#fechdef').show();
          ...
          $.ajax({
            ...
            success: function(resp) {
              ...
              $('#nombreParticipante').val(resp[0]['nombreD'] + " " + resp[0]['apellidoPaternoD'] + " " + resp[0]['apellidoMaternoD']);
              $('#titulomono').text(resp[0]['tituloMonografia']);
              if (resp[0]['aprobarMono'] == undefined || resp[0]['aprobarMono'] == 1) {
                // $('#nombreDef').val(1);
                $('#nombreDef').append('<option value="1">Primera Defensa</option>');
                $('#tipdef').show();
                $('#fechdef').show();
                ...
                if (resp[0]['aprobarMono'] == 0) {
                  ...
                  $('#fechdef').hide();
                  $('#mnsjprogdefens').show();
                  $('#mnsjprogdefens').html('<br><center><p class="text text-danger"><strong>El TUTOR NO APROBO LA MONOGRAFIA O NO CANCELO, POR LO TANTO NO PUEDE PROGRAMAR DEFENSA, VERIFIQUE EN LISTA DE TUTORIAS>EDICION</strong></p></center>');
                } else {
...
<script>
  function selectgrado(dato) {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/academico/escogerProfesion",
      data: {
        nombrep: dato,
      },
      success: function(data) {
        ...
        $.each(data, function(key, registro) {
          ...
          $('#profA').append('<option value=' + registro.idProfesion + '>' + registro.nombreP + '</option>');
        });
  ...
  function buscarciAcademico() {
    ...
    if (ciA.length > 0) {
      $.ajax({
        type: 'post',
        url: "http://localhost:8080/diplomado/index.php/academico/buscarAcademico",
        data: {
          txtCiA: ciA
        },
        success: function(data) {
          ...
          if (data > 0) {
            ...
            $("#mensajeAcademico").html('<p class="alert alert-warning"><small>Academico Registrado, Ingrese a Ver Academico.</small></p>');
            ...
            $("#mensajeAcademico").html('<p class="alert alert-success"><small>CI no encontrado, Ingrese datos del nuevo Academico.</small></p>');
      ...
      $("#mensajeAcademico").html('<p class="alert alert-danger"><small>Espere!!! Ingrese CI de Academico.</small></p>');
      $("#nombreA").attr('disabled', 'disabled');
      $("#ciudadA").attr('disabled', 'disabled');
      $("#telefonoA").attr('disabled', 'disabled');
      $("#direccionA").attr('disabled', 'disabled');
  ...
  function editarAcademico() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/academicoprofesion/obtenerProfesiones",
      data: {
        txtci: aci,
      },
      success: function(data) {
        data = JSON.parse(data);
        $.each(data, function(key, registro) {
          $('#editProfesion').append('<li value=' + registro.idProfesion + '>' + registro.nombreP + '   ' + '<span class="label label-info">' + registro.gradoAcademicoP + '</span></li>');
          ...
          //   alert('ajhd');
          // }));
          // $('#botonEliminar').append('<a onclick="llamadaAjax()"> type="button" href="http://localhost:8080/diplomado/index.php/academicoprofesion/eliminarProfesionAcad?ci='+aci+'&& prof='+registro.nombreP+'">Eliminar</a></br> ');
  ...
  function insertPosgrado() {
    ...
    $.getJSON('http://localhost:8080/diplomado/index.php/especialidad/obtenerEspecialidades', function(data) {
      console.log(data);
      $.each(data, function(key, registro) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        console.log(data);
        $.each(data, function(key, registro) {
          $('#selectModal').append('<option value=' + registro.idProfesion + '>' + registro.nombreP + '...' + registro.gradoAcademicoP + '</option>');
        });
...
<script>
  ...
  $("body").on("click", "#listaUsuarios b", function(event) {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/usuario/getUsuario",
      data: {
        ciUsuario: ciu
      },
      success: function(data) {
  ...
  function eliminarUsuario() {
    ...
    if (ciuintroducido.length > 0) {
      if (ciuintroducido == ciu) {
        $.ajax({
          type: 'post',
          url: "http://localhost:8080/diplomado/index.php/usuario/eliminarUsuario",
          data: {
          ...
          success: function(data) {
            console.log(data);
            if (data == 'seelimino') {
              $("#msjusuario").html('<p class="alert alert-success"><small>Se Elimino el Usuario!!!</small></p>').fadeIn();
              $("#eliminarUsuario").modal('hide')
            } else {
              $("#msjusuario").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-alert" ></span><small> No es posible Eliminar al Usuario!!!</small></p>').fadeIn();
              $("#eliminarUsuario").modal('hide')
            }
          },
          complete: function() {
        ...
        $("#mnsjEliminar").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> El CI que introdujo no pertenece al Usuario seleccionado!!</small></p>');
      }
    } else {
      $("#mnsjEliminar").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> Debe Introducir CI del Usuario si desea Eliminar!</small></p>');
...
<script>
  ...
  //       {
  //         type:'post',
  //         url:"http://localhost:8080/diplomado/index.php/profesion/crearProfesion",
  //         data:{
  //           data
  //         },
  //         success:function(data){
  ...
  //       {
  //         type:'post',
  //         url:"http://localhost:8080/diplomado/index.php/profesion/seleccionarProfesiones",
  //         data:{
  ...
  function selectProfesion() {
    ...
    $.ajax({
      type: 'get',
      url: "http://localhost:8080/diplomado/index.php/profesion/getAllProfesiones",
      data: {
  ...
  function crearProfesion() {
    ...
    if (nombrePr.length > 0) {
      $.ajax({
        type: 'post',
        url: "http://localhost:8080/diplomado/index.php/profesion/nuevaProfesion",
        data: {
          txtNombreP: nombrePr,
...
<script>
  ...
  $("body").on("click", "#listamodulos b", function(event) {
    ...
    $('#vigencia').html(vigencia == 'Activo' ? '<p class="alert alert-success"><strong>EN CURSO</strong></p>' : '<p class="alert alert-danger"><strong>CULMINADO</strong></p>');
  ...
  function calcularDiasentre_fechas_form_create(hasta, desde) {
    ...
    if ($('#txtFechaInicioMo').val() != 0000 - 00 - 00) {
      ...
      // console.log(fhasta);
      if (diff_in_days < 0) {
        $('#restafechasMO').html('<p class="alert alert-danger w-100"><strong>' + diff_in_days + ' Dias</strong></p>');
        $('#vigenteedit').val(0);
        // $('#btnActualizar').val('Activar Modulo');boton
      } else {
        $('#restafechasMO').html('<p class="alert alert-success w-100"><strong>' + diff_in_days + ' Dias</strong></p>');
        $('#vigenteedit').val(1);
        // $('#btnActualizar').val('Desactivar');boton
      }
    } else {
...
<script>
  ...
  function editaParalelo() {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/paralelo/existeParalelo",
      data: {
        numModulo: nummod,
      ...
      success: function(data) {
        ...
        if (data == '') {
          $.ajax({
            type: 'post',
            url: "http://localhost:8080/diplomado/index.php/paralelo/editarParalelo",
            data: {
              txtnumeromod: nummod,
            ...
            success: function(data) {
              ...
              if (data == 'noseedito') {
                ...
                // top.location.reload(true);
                $("#msjEditar").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> No es Posible Editar el Paralelo</small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();


              } else {
                $('#modalEditarParalelo').modal('hide')

                $("#msjEditar").html('<p class="alert alert-success"><span class="glyphicon glyphicon-ok" ></span><small> Se Edito datos del Paralelo Exitosamente!!!</small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
          ...
          $("#mnsjErrorEdicion").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> Ya Existe un Paralelo con el mismo Nombre!!!</small><span class="glyphicon glyphicon-hand-up" ></span></p>');
...
<script>
    ...
    if (ciD == '') {
      ...
      $("#formMonografia").validate({
        ...
        submitHandler: function(form) {
          ...
          $.ajax({
            type: "POST",
            url: "http://localhost:8080/diplomado/index.php/monografia/crearMonografia_de_Diplomante",
            data: {
              txtcidiplomante: ciD,
  ...
  $("body").on("click", "#listaMonografia b", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/monografia/getMonografiaporId",
      data: {
  ...
  function editarMonografia() {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/monografia/editarMonografia",
      data: {
      ...
      success: function(data) {
        // console.log(data);
        if (data == 'noeditMono') {
          $("#mnsjeditMono").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> No es Posible Editar Monografia</small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();

        } else {

          $("#mnsjeditMono").html('<p class="alert alert-success"><span class="glyphicon glyphicon-ok" ></span><small> Se Edito datos de Monografia Exitosamente!!!</small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
...
<script>
  ...
  function iniciarV() {
    ...
    if (confirm('Desde este momento se Iniciara la version Actual, solo podra acceder a esta version. Desea Continuar?')) {
      $.getJSON('http://localhost:8080/diplomado/index.php/version/habilitarV', function(resp) {
        console.log(resp);
...
<script>
  ...
  $("body").on("click", "#listaDefensa b", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/defensa/getDefensa_tribunal",
      data: {
        iddefensa: defensa
      },
      success: function(data) {
        data = JSON.parse(data);
        // console.log(data);
        $('#nombreD').text(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
        $('#tituloMono').text(data[0]['tituloMonografia']);
        $.each(data, function(key, registro) {
  ...
  $(document).ready(function() {
    ...
    $("#formModalDefenza").validate({
      ...
      submitHandler: function(form) {
        ...
        $.ajax({
          type: "POST",
          url: "http://localhost:8080/diplomado/index.php/defensa/crearDefensa_paraPie",
          data: {
            realizamono: realizamono,
            nombreDef: nombreDef,
            fechaDef: fechaDef,
  ...
  function editarTribunal() {
    ...
    if (academico != '') {
      $.ajax({
        type: 'POST',
        url: "http://localhost:8080/diplomado/index.php/defensa/editarTribunal",
        data: {
          tribunal: trib,
        ...
        success: function(data) {
          // console.log(data);
          if (data == 'noeditrib') {
            $(".mnsjeditribunal").html('<p class="alert alert-warning "><span class="glyphicon glyphicon-alert" ></span><small> No es posible Editar Tribunal" </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>');
          } else {
            $(".mnsjeditribunal").html('<p class="alert alert-success"><span class="glyphicon glyphicon-ok" ></span><small> Se Edito Tribunal " </small><span class="glyphicon glyphicon-hand-right" ></span></p>');
            data = JSON.parse(data);
      ...
      $(".mnsjeditribunal").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> Debe seleccionar un Academico " </small><span class="glyphicon glyphicon-hand-up" ></span></p>');
    }
  }
  ...
  $("body").on("click", "#listaDefensa c", function(event) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        // console.log(data);
        $('#defensa').val(defensa);
        $('#nombreDiplo').text(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
  ...
  function cambiarFechaDef() {
    ...
    // console.log(fecha);
    if (fecha == '') {
      $("#mnsjeditfecha").html('<p class="alert alert-warning "><span class="glyphicon glyphicon-alert" ></span><small> Debe Seleccionar una Fecha y Hora" </small><span class="glyphicon glyphicon-hand-up" ></span></p>').fadeIn();
    } else {
      $.ajax({
        type: 'POST',
        url: "http://localhost:8080/diplomado/index.php/defensa/editarFechaDef",
        data: {
          defensa: defensaa,
          fechad: fecha
        },
        success: function(data) {
          ...
          if (data == 'noeditFechaDef') {
            $("#mnsjeditfecha").html('<p class="alert alert-warning "><span class="glyphicon glyphicon-alert" ></span><small> No es posible Editar Fecha de Defensa" </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();
          } else {
            $("#mnsjeditfecha").html('<p class="alert alert-success"><span class="glyphicon glyphicon-ok" ></span><small> Se Edito Fecha de Defensa " </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
            data = JSON.parse(data);
            $('#fechaDef').text(data[0]['fechaDef']);
          }
        },
  ...
  function asignarNotaDef() {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/defensa/AsignarNotaDefensa",
      data: {
        defensa: defensaa,
        nota: nota,
      ...
      success: function(data) {
        ...
        if (data == 'nocalificadoDef') {
          $("#mnsjeditfecha").html('<p class="alert alert-warning "><span class="glyphicon glyphicon-alert" ></span><small> No es posible Registrar Resultados de Defensa" </small><span class="glyphicon glyphicon-thumbs-down" ></span></p>').fadeIn();
        } else {
          $("#mnsjeditfecha").html('<p class="alert alert-success"><span class="glyphicon glyphicon-ok" ></span><small> Se Registraron los Resulados de la Defensa " </small><span class="glyphicon glyphicon-thumbs-up" ></span></p>').fadeIn();
          data = JSON.parse(data);
          $('#notaDef').val(data[0]['notaDef']);
          $('#AprobacionDef').val(data[0]['aprobarDef']);
          $('#observDef').val(data[0]['observacionDef']);
  ...
  $("body").on("click", "#listaDefensa d", function(event) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $('#detnombreDef').text(data[0]['nombreD'] + " " + data[0]['apellidoPaternoD'] + " " + data[0]['apellidoMaternoD']);
  ...
  $("body").on("click", "#aleatorio", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/defensa/tribunal_aleatorio",
      data: {
  ...
  $("body").on("click", "#listaAcademicosmodal c", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/academico/getAcademico_paraModalDefenza",
      data: {
        ciAcademico: ci
      },
      success: function(data) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/academico/getAcademicoProfesion_paramodalDef",
      data: {
        ciAcademico: ci
      },
      success: function(data) {
        if (data == "listaVaciaAcademicoProfesion") {
          ...
          $.each(data, function(key, registro) {
            ...
            $("#profesionAcademico").append('<li type="button"><small>' + registro.nombreP + '<span class="label label-default" > ' + registro.gradoAcademicoP + '</span></small></li>');
            // $('#profesiones').append('<option value='+registro.idProfesion+'>'+registro.nombreP+'--'+registro.gradoAcademicoP+'</option>');
          });
        }
      }
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/academico/getAcademicoCompleto_paramodalDef",
      data: {
        ciAcademico: ci
      },
      success: function(data) {
        if (data == "listaVaciaAcademicoEspecialidad") {
          ...
          $.each(data, function(key, registro) {
            console.log(registro);
            $("#academicoCompleto").append('<tr><td><small>' + registro.nombreP + '--' + registro.gradoAcademicoP + '</small></td></br> <td><small>' + registro.nombreE + '</small></td></tr>');
            // $('#profesiones').append('<option value='+registro.idProfesion+'>'+registro.nombreP+'--'+registro.gradoAcademicoP+'</option>');
          });
        }
      }
  ...
  function selecTribunal() {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/academicoseleccionado/crearSeleccionAcademico",
      data: {
        citrib: ciTribunal
      },
      success: function(data) {
        if (data == "nosepuedeinsertaracademico") {
          ...
          switch (tipoTrib) {
            case "Presidente":
              ...
              $("table td:nth-child(2):contains(" + v1 + ")").parents("tr").css("background-color", "#5499C7 ");
            ...
            case "Secretario":
              ...
              // $("table td").parents("tr").css("background-color","white");
              $("table td:nth-child(2):contains(" + v1 + ")").parents("tr").css("background-color", "#F4D03F");
              $("#botonForm").removeAttr('disabled'); //desbloquea el boton "Registrar defensa"
              break;
            default:
  ...
  function terminarVersion() {
    ...
    if (cidirector.length > 0) {
      $.getJSON('http://localhost:8080/diplomado/index.php/usuario/getDirector', function(data) {
        // data=JSON.parse(data);
        console.log('el id del la bd es' + data[0]['ciU']);
        directorbd = data[0]['ciU'];

        if (cidirector == directorbd) {
          $("#mnsjEliminarVersion").html('<p class="alert alert-success"><span class="glyphicon glyphicon-thumbs-up" ></span><small> el CI si coincide!</small></p>');
          console.log(version);
          $.ajax({
            type: 'post',
            url: "http://localhost:8080/diplomado/index.php/version/cerrar_sesion_version",
            data: {
              version: version,
              razonV: razon
            },
            success: function(data) {
              ...
              if (data == 'versiontermino') {
                $("#mnsjEliminarVersion").html('<p class="alert alert-danger"><small>YA SE CERRÓ LA VERSIÓN!!!</small></p>').fadeIn();
                ...
                window.location.href = "http://localhost:8080/diplomado/index.php/version/imprimirtcpdf/" + data;
              }
            },
            // complete:function() {
            //   setTimeout(function(){
          ...
          $("#mnsjEliminarVersion").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> El CI que introdujo no pertenece al Usuario seleccionado!!</small></p>');
        }
      });

    } else {
      $("#mnsjEliminarVersion").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-alert" ></span><small> Debe Introducir su CI para comprobar que puede realizar esta acción!</small></p>');
  ...
  $("body").on("click", "#espacio c", function(event) {
    ...
    $.ajax({
      type: 'post',
      url: "http://localhost:8080/diplomado/index.php/version/getEvento_vercion",
      data: {
      ...
      success: function(data) {
        ...
        if (data == "nohayevento") {
          ...
          $.each(data, function(key, registro) {
            ...
            $('#nombrev').append('<span><small>' + registro.nombreV + '</small></span></br></br>');
            if (registro.estadoVersion == 0) {
              $('#accionv').append('<span class="text text-danger"><small>Desabilitó</small></span></br></br>');
            } else {
              $('#accionv').append('<span class="text text-success"><small>Habilitó</small></span></br></br>');
            }
            ...
            $('#imprimirv').append('<a class="btn btn-primary btn-xs" href="http://localhost:8080/diplomado/index.php/version/imprimirtcpdf/?bversion=' + registro.idBitacora_version + '"><span class="glyphicon glyphicon-print" ></span></a></br></br>');
            num += 1;
    ...
    if (cidirector.length > 0) {
      $.getJSON('http://localhost:8080/diplomado/index.php/usuario/getDirector', function(data) {
        ...
        if (cidirector == directorbd) {
          ...
          $.ajax({
            type: 'post',
            url: "http://localhost:8080/diplomado/index.php/version/reiniciar_sesion_version",
            data: {
              version: version,
              razonV: razon
            },
            success: function(data) {
              ...
              if (data == 'yasehabilitoversion') {
                $("#mnsjReiniciarVersion").html('<p class="alert alert-danger"><small>YA SE HABILITO LA VERSIÓN!!!</small></p>').fadeIn();
                // $("#terminarVersion").modal('hide')
              } else {
          ...
          $("#mnsjReiniciarVersion").html('<p class="alert alert-warning"><span class="glyphicon glyphicon-alert" ></span><small> El CI que introdujo no pertenece al Usuario autorizado!!</small></p>');
        }
      });

    } else {
      $("#mnsjReiniciarVersion").html('<p class="alert alert-danger"><span class="glyphicon glyphicon-alert" ></span><small> Debe Introducir su CI para comprobar que puede realizar esta acción!</small></p>');
  ...
  function eliminardowload(archivo) {
    if (confirm('Esta seguro de ELIMINAR el Documento de Calificaciones?? Una vez que acepte se volvera a Habilitar la Edicion de Calificaciones!!!')) {
      $.ajax({
        type: 'post',
        url: "http://localhost:8080/diplomado/index.php/cargarcalificacionpdf/eliminar_download",
        data: {
          pdf: archivo
        },
        success: function(data) {
          ...
          switch (data) {
            case 'nohayarchivo':
              $("#mnsjeliminopdf").html('<p class="alert alert-danger"><small>No existe archivo en la base de datos!!!</small></p>').fadeIn();
              break;
            case 'nosepuedeeliminarpdf':
              $("#mnsjeliminopdf").html('<p class="alert alert-danger"><small>No se puede eliminar el Archivo de Calificaciones!!!</small></p>').fadeIn();
              break;
            case 'siseeliminopdf':
              $("#mnsjeliminopdf").html('<p class="alert alert-success"><span class="glyphicon glyphicon-up" ></span><small> Se Elimino el archivo de calificaciones !!!</small></p>').fadeIn();
              break;
            case 'noeliminadeBD':
              $("#mnsjeliminopdf").html('<p class="alert alert-danger"><small>No se puede Eliminar de la base de datos!!!</small></p>').fadeIn();
              break;
            default:
              $("#mnsjeliminopdf").html('<p class="alert alert-danger"><small>' + data + '</small></p>').fadeIn();
              break;
          }
        },
        complete: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (rutamono == null) {
          ...
          $('#descargarmono').attr("href", "http://localhost:8080/diplomado/index.php/monografia/downloadmonografiapdf/" + rutamono);
  ...
  $("#formMonografiapdf").submit(function(e) { //puede funcionarVERRRRR O REVISAAAAR
    ...
    $.ajax({
      url: "http://localhost:8080/diplomado/index.php/monografia/subirMonografiapdf",
      type: "POST",
      data: formdata,
      ...
      success: function(resp) {
        ...
        switch (resp) {
          case 'nosubiomonopdf':
            $("#mnsjpdfMono").html('<p class="alert alert-danger"><small>No se pudo subir el Archivo Monografia!!!</small></p>').fadeIn();
            break;
          case 'subiomonopdf':
            $("#formMonografiapdf").hide();
            $("#idcargado").show();
            $("#mnsjpdfMono").html('<p class="alert alert-success"><small>Se cargo exitosamente el archivo Monografia</small></p>').fadeIn();
            break;
          case 'noseleccpdf':
            $("#mnsjpdfMono").html('<p class="alert alert-danger"><small>Selecciones un Documento pdf!!!</small></p>').fadeIn();
            break;
          default:
            $("#mnsjpdfMono").html('<p class="alert alert-danger"><small>' + resp + '</small></p>').fadeIn();
            break;
        }
      },
      complete: function() {
  ...
  function eliminarmonopdf() {
    if (confirm('Esta Seguro de Eliminar el Documento de Monografia del Diplomante?')) {
      ...
      $.ajax({
        type: 'post',
        url: "http://localhost:8080/diplomado/index.php/monografia/eliminar_monografiapdf",
        data: {
          pdf: archivo
        },
        success: function(data) {
          ...
          switch (data) {
            case 'nohayarchivomono':
              $("#mnsjeditMono").html('<p class="alert alert-danger"><small>No existe Monografia en la base de datos!!!</small></p>').fadeIn();
              break;
            case 'nosepuedeeliminarpdfmono':
              $("#mnsjeditMono").html('<p class="alert alert-danger"><small>No se puede eliminar el Documenti Monografia!!!</small></p>').fadeIn();
              break;
            case 'siseeliminopdfmono':
              $("#formMonografiapdf").show();
              $("#idcargado").hide();
              $("#mnsjeditMono").html('<p class="alert alert-success"><span class="glyphicon glyphicon-up" ></span><small> Se Elimino el Documnto Monografia  !!!</small></p>').fadeIn();
              break;
            case 'noeliminadeBDmono':
              $("#mnsjeditMono").html('<p class="alert alert-danger"><small>No se puede Eliminar de la base de datos!!!</small></p>').fadeIn();
              break;
            default:
              $("#mnsjeditMono").html('<p class="alert alert-danger"><small>' + data + '</small></p>').fadeIn();
              break;
          }
        },
        complete: function() {
  ...
  $("body").on("click", "#listaMonografia d", function(event) {
    ...
    $.ajax({
      type: 'POST',
      url: "http://localhost:8080/diplomado/index.php/monografia/comparar_monografiapdf",
      data: {
        monografia: idmono
      },
      success: function(data) {
        console.log(data);
        if (data == 'nohaymonog') {
          $("#mnsjcompararmono").html('<p class="alert alert-danger"><small>No existen Monografias en la base de datos!!!</small></p>');
        } else {
          // resp=JSON.parse(resp);
...
<script>
  ...
  $(document).ready(function() {
    ...
    $.getJSON('http://localhost:8080/diplomado/index.php/calificacion/getProbabilidad_calificaciones', function(data) {
      // data=JSON.parse(data);  
      console.log(data);
      modulo = data['modulo'];
    ...
    $.getJSON('http://localhost:8080/diplomado/index.php/monografia/grafic_fecha_defensas', function(data) {
      // data=JSON.parse(data);  
      console.log(data);
      def1 = data[0];
      def2 = data[1];
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#SelectDescuentos').change(function() {
      ...
      var porcentajeDescuento = parseInt($(this).val()); // Obtener el porcentaje de descuento seleccionado
      var costoEvento = parseFloat(<?= $version->costoModulosV; ?>); // Obtener el costo del evento desde PHP
      var descuentoCalculado = costoEvento * (porcentajeDescuento / 100); // Calcular el descuento en base al porcentaje

      var totalCostoFinalColegiatura = costoEvento - descuentoCalculado; // Calcular el costo final

      var totalCostoFinal = totalCostoFinalColegiatura + parseFloat(<?= $version->costoMatriculaV; ?>); // Calcular el costo final
