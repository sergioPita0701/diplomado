...
$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.'Lista de Defensas en la '.$nombre, PDF_HEADER_STRING);
...
$tabla='<table border="1" cellspacing="3" cellpadding="4">
    ...
    $no=1;
    foreach($listadefensas as $filita ) {
    $defe=($filita['nombreDef']=="1")? "Primera Def.":"Recuperatorio Def.";
    $trib1=($filita['tipo_tribunal']=="Presidente")? $filita['nombreA'] : "";
    $trib2=($filita['tipo_tribunal']=="Secretario")? $filita['nombreA'] : "";
$tabla.=        
    '<tr>
        <td>'.$no++.'</td>
        <td>'.$filita['fechaDef'].'</td>
        <td align="center">'.$defe.'</td>
        <td align="center">'.$filita['ciD'].'</td>
        <td align="center">'.$filita['nombreD'].' '.$filita['apellidoPaternoD'].'</td>
        <td align="center">'.$trib1.'</td>
        <td align="center">'.$trib2.'</td>
    </tr>';
    }
